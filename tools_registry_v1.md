...
## 7c. Инструменты: заметки (внеплановая задача 2026-07-31)

Мотивация: память фактов write-only для модели (§7.2 — автоинжект top-5), поэтому запросы вида «покажи все пробежки и сгруппируй по дистанциям» невыполнимы: полный структурированный список фактов модель прочитать не может и честно отказывает. Накапливаемые записи одной темы (лог тренировок, список идей) — не атомарные факты; вести их в memory_facts — злоупотребление (вытесняют настоящие факты из лимита 500 и замусоривают автоинжект). Решение владельца 2026-07-31: отдельная сущность «заметки» — тематические документы с явным чтением через инструменты, без автоинжекта в промпт. Реализация — делегированием внутри Claude (Codex недоступен, указание владельца 2026-07-31).

Хранение — таблица `notes` (db-schema.md): `UNIQUE (user_id, title)`, адресация по title (модель не оперирует id — нет галлюцинаций идентификаторов). Лимиты: **100 заметок на пользователя**, title ≤64 символов, content ≤8000 символов (после trim). Санитизация строк как в §7a.1: trim, схлопывание CR/LF в title, обрезка с retryable-ошибкой при превышении длины title; превышение content — отдельная семантика, см. 7c.2/7c.6. Все mutating-инструменты идемпотентны по стандартному ключу §1.7, все вызовы — в tool_calls_log.

### 7c.1 upsert_note — MUTATING_INTERNAL
```python
class UpsertNoteArgs(BaseModel):
    title: str                     # тема заметки, ≤64 символов после trim
    content: str                   # полный новый текст, ≤8000 символов
```
Создаёт заметку или **полностью заменяет** содержимое существующей (UPSERT по `(user_id, title)`). Правило для модели (спека промпта 16g): для добавления записей использовать append_to_note; upsert — только создание новой темы или явная просьба переписать. payload: `{"created": true|false, "notes_count": N, "notes_limit": 100}`. daily_limit: 30.

### 7c.2 append_to_note — MUTATING_INTERNAL
```python
class AppendToNoteArgs(BaseModel):
    title: str                     # тема существующей или новой заметки
    text: str                      # дописываемая запись, ≤1000 символов
```
Ключевой инструмент сценария «лог пробежек»: дописывает `text` новой строкой в конец content (разделитель `\n`); заметки нет — создаёт с content=text (модели не нужен предварительный upsert). Резолв title — как в 7c.4 (сначала точный, затем case-insensitive): иначе «пробежки» и «Пробежки» молча раздваивают лог (уточнение по итогам реализации 2026-07-31; upsert_note сознательно остаётся точным — он цель UPSERT). Итоговый content превысил бы 8000 символов → `status="error"`, `retryable=False`, текст: «Заметка "{title}" переполнена ({n}/8000 символов). Предложите пользователю ревизию: сократить её, разбить на части или удалить лишнее.» payload при успехе: `{"created": bool, "note_chars": n, "note_chars_limit": 8000, "notes_count": N, "notes_limit": 100}`. daily_limit: 60.

### 7c.3 list_notes — READ_ONLY
Без аргументов. Возвращает все заметки пользователя: `[{"title", "updated_at", "chars"}]`, сортировка по updated_at DESC, плюс `notes_count`/`notes_limit`. Content не возвращается (бюджет tool result). Лимита нет.

### 7c.4 read_note — READ_ONLY
```python
class ReadNoteArgs(BaseModel):
    title: str
```
Полный текст заметки. Точного совпадения title нет → `status="error"`, `retryable=True`, в тексте ошибки — до 10 ближайших существующих заголовков (сравнение case-insensitive по lower(trim)), модель выбирает и повторяет. У пользователя вообще нет заметок → `retryable=False` («У пользователя пока нет заметок») — повторять не по чему, ретраи модели беречь (уточнение 2026-07-31). Лимита нет.

### 7c.5 delete_note — MUTATING_INTERNAL
```python
class DeleteNoteArgs(BaseModel):
    title: str
```
Удаление по точному title (case-insensitive резолв как в 7c.4). Правило для модели (16i): удалять только по явной просьбе или явному согласию пользователя на ревизию. payload: `{"notes_count": N}`. daily_limit: 30.

### 7c.6 Переполнение лимитов и ревизия (решение владельца 2026-07-31)

Бот обязан просить провести ревизию заметок; ревизию можно доверить боту или провести руками пользователя. Механика — без новой инфраструктуры, силами существующих инструментов:

1. **Мягкое предупреждение с 90%**: `notes_count` ≥ 90 в payload любой мутации → правило промпта (16j) велит один раз предложить пользователю ревизию (не в каждом сообщении — без спама).
2. **Жёсткий отказ на 101-й заметке**: upsert_note/append_to_note с новым title при `notes_count` = 100 → `status="error"`, `retryable=False`, текст: «Достигнут лимит 100 заметок. Предложите пользователю ревизию: показать список заметок (list_notes) и удалить или объединить неактуальные — сам или поручив вам.» Дописывание в существующие заметки продолжает работать.
3. **Автоматическая ревизия** = модель по согласию пользователя («почисти сам»): list_notes → read_note по кандидатам (старые updated_at, мелкие chars) → предложить конкретный план (что удалить, что объединить) → после подтверждения delete_note/upsert_note. Правило 16k: без явного согласия пользователя ничего не удалять; план ревизии всегда проговаривается до выполнения.
4. **Ручная ревизия** = пользователь сам называет, что удалить; бот выполняет delete_note.

Вытеснения по давности (как в memory_facts) нет сознательно: заметка — явный пользовательский контент, молчаливое удаление недопустимо.

### 7c.7 Разграничение с памятью фактов

remember_fact — атомарные устойчивые факты о пользователе (автоинжект, фон). notes — накапливаемые/структурированные записи по темам с явным чтением. Правило выбора — в спеке промпта (16g/16h); реестр фактов не меняется.

Definition of done: миграция таблицы notes с RLS; пять инструментов в core-профиле реестра с записью в tool_calls_log; санитизация и лимиты по этому разделу; секция NOTES в обеих версиях промпта (v1, v2-flash) с правилами 16g–16k спеки; юнит-тесты (upsert/append/создание через append, переполнение content, отказ на 101-й, резолв title с подсказками, идемпотентность переигровки, RLS-изоляция); eval-сценарии заметок зелёные на deepseek-v4-flash (указание владельца 2026-07-31 — приёмочный прогон именно на прод-модели); живой e2e через диспетчер.
...
| upsert_note | internal | 100 актив., 30/день | core | внепл. 2026-07-31 |
| append_to_note | internal | 60/день | core | внепл. 2026-07-31 |
| list_notes | read | — | core | внепл. 2026-07-31 |
| read_note | read | — | core | внепл. 2026-07-31 |
| delete_note | internal | 30/день | core | внепл. 2026-07-31 |
