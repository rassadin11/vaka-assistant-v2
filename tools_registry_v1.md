...
## 7a. Инструменты: персона ассистента (внеплановая задача 2026-07-18)

### 7a.1 set_assistant_persona — MUTATING_INTERNAL
```python
class SetAssistantPersonaArgs(BaseModel):
    name: str | None = None        # имя ассистента, ≤30 символов после trim
    address: Literal["ty", "vy"] | None = None   # обращение к пользователю: ты/вы
    style: str | None = None       # свободные пожелания тона, ≤200 символов после trim
    gender: Literal["female", "male", "neutral"] | None = None  # грамматический род самоотсылок (итерация 2, спека assistant-persona-iteration-2.md)
```
Частичное обновление: непереданные поля сохраняют прежние значения; передача поля обновляет его. Санитизация всех строк: trim, схлопывание переносов строк в пробелы, обрезка по лимиту с ошибкой retryable при превышении (модель переформулирует короче). Хранение — `users.assistant_profile` (jsonb) через user_transaction (RLS). Инжекция — блок C2 системного промпта (спека system-prompt-spec.md), детерминированно в каждый запрос при непустом профиле. daily_limit: 20 (защита от промпт-качелей). Идемпотентность — стандартный ключ §1.7.

### 7a.2 clear_assistant_persona — MUTATING_INTERNAL
Без аргументов; `assistant_profile = NULL`. Возвращает подтверждение сброса.

Definition of done: миграция колонки; оба инструмента в реестре core-профиля; блок C2 в контекст-менеджере с бюджетом ≤200 токенов; правило-рамка в статичном ядре промпта (16a–16c спеки); eval-сценарии персоны зелёные; записи в tool_calls_log.

---

## 7b. Инструмент: часовой пояс пользователя (внеплановая задача 2026-07-20)

Мотивация: `users.timezone` задаётся один раз при онбординге и после этого не меняется ничем — пользователь, который переехал или ошибся кнопкой, остаётся с неверным поясом навсегда (напоминания приходят не вовремя). Решение владельца 2026-07-20: (а) инструмент, чтобы ассистент менял пояс сам по фразе пользователя; (б) команда `/timezone` как детерминированный запасной путь.

### 7b.1 set_timezone — MUTATING_INTERNAL

```python
class SetTimezoneArgs(BaseModel):
    city: str                      # город или IANA-идентификатор, ≤64 символов после trim
```

Модель передаёт **город своими словами** («Новосибирск», «новосибирск», «Asia/Novosibirsk»). Резолв — строго кодом, модель не выбирает идентификатор зоны:

1. trim + lower + схлопывание пробелов → поиск в `CITY_TZ` (worker/onboarding.py, 61 город);
2. промах → если строка совпадает с элементом `VALID_TIMEZONES` (регистр как есть) — принять её;
3. иначе — `status="error"`, `retryable=True`, текст «Не узнал город. Назовите крупный город рядом — например, Самара.» (модель переспрашивает пользователя).

Запрет выдумывания зон обязателен: без валидации модель пишет несуществующие идентификаторы вида `Europe/Omsk`, и `ZoneInfo` роняет планировщик на следующем тике.

Запись — `UPDATE users SET timezone` через `user_transaction` (RLS), в той же транзакции считается число затронутых повторяющихся напоминаний:
...
payload: `{"timezone": "Asia/Novosibirsk", "local_time": "2026-07-20 18:34", "weekday": "monday", "recurring_reminders": 3}`.

`local_time` в payload обязателен: системный промпт с блоком `Current time / Timezone` собирается один раз на задачу (`worker/agent_processor.py`, `build_context` из `TaskContext`), и после смены пояса он остаётся старым до конца задачи. Модель обязана считать время от результата инструмента, а не от шапки — правило добавляется в статичное ядро промпта (см. DoD ниже).

Смена пояса на тот же самый — не ошибка: `status="ok"`, `payload["unchanged"]=true`, БД не трогаем.

daily_limit: **3**. Обоснование: дневные счётчики лимитов и ₽-бюджета кладутся в Redis по локальной дате (`core/limits.py`, `core/spend.py`, ключи `msg:{user_id}:{YYYYMMDD}` и `spend_rub:{...}`), поэтому смена пояса может перепрыгнуть дату и обнулить дневной расход. При 3 сменах в сутки это неинтересно как способ обойти лимит; переезжают реже. Перед 7.4 (тариф v1) перепроверить, достаточно ли этого, когда лимиты начнут защищать деньги.

Идемпотентность — стандартный ключ §1.7 (переигровка `update_id` не приводит к повторной записи).

### 7b.2 Поведение существующих напоминаний

**Решение владельца 2026-07-20: напоминания следуют за пользователем.** Повторяющиеся напоминания пересчитываются планировщиком по `croniter` в текущей `users.timezone` на каждом срабатывании (`worker/scheduler.py`) — то есть «каждый понедельник в 9» после смены пояса начинает приходить в 9 утра по новому месту. Это происходит само, кода не требует.

Схема **не меняется**: колонки таймзоны у `scheduled_tasks` не будет, привязать отдельное напоминание к старому поясу нельзя. Альтернатива (пояс на каждое напоминание) отклонена как избыточная: переезд редок, а UI выбора пояса на каждое напоминание не окупается.

Одноразовые напоминания за пользователем **не следуют** — у них абсолютный `next_run_at` в UTC, сдвигать его задним числом нельзя (момент уже согласован с пользователем). Это расхождение обязано быть проговорено вслух: бот сообщает число сдвинутых повторяющихся напоминаний из payload. Правило для модели — в статичном ядре промпта.

### 7b.3 Команда /timezone

Детерминированный путь мимо модели, в `OnboardingProcessor._process_active_user` рядом с `/help` и `/start`: ставит ключ `onboarding:tz_pending:{tg_user_id}` (существующий TTL 7 дней) и шлёт `TIMEZONE_BUTTONS` с текстом

```
Ваш часовой пояс сейчас: {tz}. Выберите новый — по разнице с Москвой:
```

Регистрируется в `gateway/__main__.py::_set_my_commands` («Сменить часовой пояс») — после деплоя выполнить `python -m gateway set-my-commands` (runbook).

Попутно чинится существующий дефект: обработка callback `tz:` не проверяет, идёт ли онбординг, поэтому нажатие на **старую** клавиатуру из приветствия меняет пояс и повторно шлёт полное приветствие с CTA. После правки `_save_timezone` шлёт два welcome-сообщения только когда ключ `tz_pending` был выставлен онбордингом (первичная настройка); при смене пояса у активного пользователя — одно короткое подтверждение:

```
Часовой пояс обновлён: {tz}, сейчас у вас {local_time}.
```

Если при этом есть повторяющиеся напоминания, к подтверждению добавляется предложение (и то же самое — в ответе инструмента):

```
Повторяющиеся напоминания (сейчас их {n}) теперь будут приходить по новому времени.
```

Definition of done: инструмент в реестре core-профиля с валидацией города и записью в `tool_calls_log`; правило про доверие результату инструмента и про озвучивание сдвига напоминаний — в статичном ядре промпта; команда `/timezone` и её регистрация в `set_my_commands`; повторное приветствие при смене пояса не шлётся; юнит-тесты (резолв города/IANA/мусора, unchanged, счётчик повторяющихся, daily_limit) и живой e2e смены пояса с проверкой пересчёта повторяющегося напоминания планировщиком.
...
5. Уведомление: «Документ обработан, N страниц. Спрашивайте.» + подсказка, что точечные вопросы (страница/раздел/тема) работают лучше просьбы пересказать весь файл сразу (RAG-поиск по чанкам; правка 2026-07-17 по решению владельца). Пользователь может сразу попросить суммаризацию — тогда чанки первых ~30k токенов идут одним проходом.
...
| set_assistant_persona | internal | — | core | внепл. 2026-07-18 |
| clear_assistant_persona | internal | — | core | внепл. 2026-07-18 |
| set_timezone | internal | 3/день | core | внепл. 2026-07-20 |
