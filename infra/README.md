...
Действия при инцидентах (падение воркеров, очередь, LLM, восстановление из бэкапа) — [docs/runbook.md](../docs/runbook.md).

## Алерты

Перед запуском с доставкой алертов в Telegram задайте переменные окружения:
...
Без токена Grafana запускается нормально, алерты видны в UI, но доставка уведомлений молча не работает. Если dev-стек запущен без профиля `ml`, предупреждение для probe сервиса embeddings ожидаемо: сервис в этом профиле не запускается.
