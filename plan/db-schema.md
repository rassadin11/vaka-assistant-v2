...
- **FK `user_id → users(id) ON DELETE CASCADE`** на всех пользовательских таблицах — механика GDPR-удаления (этап 7.5) одной командой.
- **Роли БД**: `migrator` (владелец таблиц, только Alembic), `app` (рабочая, RLS активна), `service` (BYPASSRLS — шедулер-тик, outbox-процессор, фоновый OAuth-refresh, админ-команды, broadcast; всё, что по определению работает поперёк пользователей).
- **RLS-шаблон**: `USING (user_id = current_setting('app.user_id')::uuid)` на каждой таблице с колонкой user_id; для users — `USING (id = current_setting('app.user_id')::uuid)`. CI-тест: каждая таблица с колонкой user_id имеет политику.
