...
- Этап 2 закрыт целиком; ревизия после этапа 2 выполнена; 3.1–3.3 приняты. Следующий пункт: **3.4 (контекст-менеджер + системный промпт по plan/system-prompt-spec.md, приёмка eval ≥18/20)**. Правило: лимиты Codex кончились → передача сюда, коммит, стоп (не подменять Codex Claude'ом). Урок ночного автозапуска 03:00: фоновая сессия зависла на запросе разрешения на Edit — для автозапусков нужен режим с автоподтверждением правок.
