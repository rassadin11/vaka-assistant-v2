...
**Принято в объёме механизма 2026-07-16 (коммиты 43b69f3 + 3954f8e), засев ключей Б4 — ждёт владельца.** Код: Codex + приёмка fable (правки: диагностика причины в fail-closed ветке entrypoint; сид-лист bootstrap вынесен в `BOOTSTRAP_SEED_KEYS` — захардкоженный dev-список молча игнорировал прод-инпут, поймано живой приёмкой). Живьём на 31.76.15.130: секреты infra.env/userlist/DSN сгенерированы на сервере (значения не покидали сервер); postgres первый старт + ALTER ROLE боевые пароли + `select current_user` через pgbouncer = app; infisical-стек healthy; прод-bootstrap — org/project/env prod, 3 identities verification=PASS, засеяно 6 секретов; DoD-тест: entrypoint из app-образа с identity воркера загрузил все 6 и exec'нул команду. ruff/mypy/pytest (257 passed) зелёные, CI зелёный, gitleaks (на VPS) чисто. Решение: gateway и worker используют одну identity (worker) через общий bootstrap.env — разделить при необходимости. Осталось: досев OPENROUTER_API_KEY/TELEGRAM_BOT_TOKEN/GROQ_API_KEY/ADMIN_TELEGRAM_IDS после Б4 (повторный запуск bootstrap с расширенным BOOTSTRAP_SEED_KEYS).

#### Детализация 1.12.3 — deploy-workflow (2026-07-16, fable, перед кодом)

**Решения:**
- **Триггер — только `workflow_dispatch`** с input `image_tag` (default `latest`; для пина/отката — `sha-<full_sha>`). Осознанный деплой без watchtower — по плану.
- **Механизм пина тега на сервере:** `/etc/assistant/deploy.env` (deploy:deploy 0640; шаблон в роли app с `APP_IMAGE=`-строкой, force: no) + systemd-unit получает `EnvironmentFile=-/etc/assistant/deploy.env` (дефис — файл опционален) — тогда и деплой из CI, и автозапуск после reboot стартуют один и тот же пин. Compose-интерполяция берёт APP_IMAGE из окружения процесса (unit) или из `--env-file` при ручном запуске.
- **SSH-доступ CI:** отдельная пара ключей; публичный — в `authorized_keys` deploy (кладу я на сервере), приватный — repo secret `DEPLOY_SSH_KEY` + `DEPLOY_HOST`=31.76.15.130 (**добавляет владелец** в Settings → Secrets → Actions; gh/PAT у меня нет).
- **Шаги workflow:** checkout → ssh (webfactory/ssh-agent или appleboy/ssh-action — на выбор Codex, проще appleboy): (1) записать `APP_IMAGE=ghcr.io/rassadin11/vaka-assistant-v2:<tag>` в deploy.env (предыдущее значение — в deploy.env.prev для отката); (2) `docker compose -f /opt/assistant/compose.prod.yml pull` (с env); (3) **миграции ролью migrator до рестарта**: `docker run --rm --network personal-assistant-prod_default --env-file /etc/assistant/migrator.env <APP_IMAGE> alembic upgrade head` — переменную для alembic взять из фактического migrations/env.py (не выдумывать; migrator.env уже на сервере, DSN мимо pgbouncer); (4) `sudo systemctl restart assistant` (unit = compose up -d; sudo NOPASSWD у deploy уже есть); (5) **гейт по здоровью**: цикл до 5 мин — `docker inspect` health контейнеров gateway и caddy = healthy, иначе exit 1 (workflow красный).
- **Откат:** повторный запуск workflow с `image_tag` из deploy.env.prev; строка-инструкция в README workflow.
- **Первый запуск полного стека** — часть живой приёмки 1.12.3: gateway/worker стартуют и здоровы ещё ДО Б4 (токены нужны только для реальных отправок в Telegram/LLM; webhook — 1.11.4 после Б4).
- enable юнита: `systemctl enable assistant` — включаю при живой приёмке (это и был отложенный хвост 1.10.4/1.12.3).

**DoD 1.12.3:** workflow в репо, yamllint/CI зелёные; роль app кладёт deploy.env-шаблон и unit с EnvironmentFile (molecule verify обновлён, повторный прогон 0 changed); на сервере: запуск workflow (после добавления секретов владельцем) прогоняет pull → миграции (alembic head, 13 таблиц) → рестарт → healthy-гейт зелёный; unit enabled; `docker ps` — весь стек healthy; повторный деплой того же тега идемпотентен. Если секреты CI ещё не добавлены владельцем — та же последовательность выполняется вручную с сервера (скрипт шагов 1–5), workflow принимается позже первым реальным прогоном.

**Принято 2026-07-16 (коммиты 9371705 + ce04f02) в объёме ручного прогона; первый CI-прогон workflow — после добавления владельцем секретов DEPLOY_HOST/DEPLOY_SSH_KEY** (ключ сгенерирован, публичный уже в authorized_keys deploy; приватный — C:/Users/Artem/.assistant/deploy-ci-key). Приёмка нашла и починила: (1) в app-образе не было alembic.ini (`COPY alembic.ini`); (2) DSN migrator.env требует схему `postgresql+psycopg://` (env.py на psycopg v3; поправлено на сервере, отражено здесь). Grafana падала на пустом TELEGRAM_ALERT_* → **1B-x3 закрыт заодно**: токен рабочего бота (@vAssistantv2workbot) + chat_id владельца засеяны в infra.env с машины владельца по SSH, grafana healthy. Живой прогон полной последовательности: pull → alembic upgrade head (v1, 39 таблиц с партициями) → systemctl restart → весь стек 17 контейнеров healthy; unit enabled; **reboot-тест: после перезагрузки сервера стек поднялся сам, все контейнеры healthy, HTTPS отвечает** (это же закрывает reboot-часть 1B-x4). gateway/worker живут без API-ключей (нужны только для реальных вызовов) — полноценный smoke после Б4.
